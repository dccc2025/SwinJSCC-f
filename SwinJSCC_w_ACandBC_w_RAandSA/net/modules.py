...
class WindowAttention(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        assert (dim % num_heads) == 0, "The number of input features (in_features) are not divisible by the number of heads (number_of_heads)."
        ...
        relative_coords = relative_coords.permute(1, 2, 0).reshape(-1, 2).float()
        ...
        self.meta_network: nn.Module = nn.Sequential(
            ...
            nn.Linear(in_features=256, out_features=num_heads, bias=True),
        )
    ...
    def forward(self, x, add_token=True, token_num=0, mask=None):
        ...
        # L2 norm
        q_norm = q.norm(dim=-1, keepdim=True) + 1e-6
        k_norm = k.norm(dim=-1, keepdim=True) + 1e-6
        # q = q * self.scale
        # attn = (q @ k.transpose(-2, -1))  # (N+1)x(N+1)
        attn = (q @ k.transpose(-2, -1) / (q_norm @ k_norm.transpose(-2, -1)))
        ...
        relative_position_bias: torch.Tensor = self.meta_network(self.relative_coords_log)
        relative_position_bias: torch.Tensor = relative_position_bias.permute(1, 0) 
        relative_position_bias = relative_position_bias.view(self.num_heads, self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1])
        relative_position_bias.unsqueeze(0)

        if add_token:
            attn[:, :, token_num:, token_num:] = attn[:, :, token_num:, token_num:] + relative_position_bias
        else:
