...
class SnrModulation(nn.Module): # 等下回来 把这个跑通！然后休息！
    def __init__(self, hidden_dim):
        super(SnrModulation, self).__init__()
        self.hidden_dim = hidden_dim

        self.sm_list = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim * 2), # N->2N
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 2, self.hidden_dim * 4), # 2N->4N
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 4, self.hidden_dim * 8), # 4N->8N
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 8, self.hidden_dim * 4), # 8N->4N
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 4, self.hidden_dim * 2), # 4N->2N
            nn.ReLU(),
            nn.Linear(self.hidden_dim * 2, self.hidden_dim), # 2N->N
            nn.Sigmoid()
        )
...
class SwinJSCC_Encoder(nn.Module):
    def __init__(self, img_size, patch_size, in_chans,
        ...
        self.hidden_dim = int(self.embed_dims[len(embed_dims)-1])
        self.layer_num: int = 7
        ...
        # re-construct sm
        self.sm_rise_dim = nn.Linear(self.embed_dims[len(embed_dims) - 1], self.hidden_dim) # C->N
        self.sm_noise_modulator = AdaptiveModulator(self.hidden_dim) # snr: 1->NxN
        self.sm = SnrModulation(hidden_dim=self.hidden_dim)

        # re-construct bm
        self.bm_rise_dim = nn.Linear(self.embed_dims[len(embed_dims) - 1], self.hidden_dim) # C->N
        self.bm_noise_modulator = AdaptiveModulator(self.hidden_dim) # snr: 1->NxN
        self.bm = SnrModulation(hidden_dim=self.hidden_dim)
    ...
    def forward(self, x, snr, rate, model):
        ...
        if model == 'SwinJSCC_w/o_SAandRA':
            ...
            snr_batch = snr_cuda.unsqueeze(0).expand(B, -1) # [B, 1]

            csi = self.sm_noise_modulator(snr_batch).unsqueeze(1).expand(-1, H * W // (self.num_layers ** 4), -1)
            temp = self.sm_rise_dim(x.detach()) * csi # [B, N, N], N=H x W/(self.num_layers**4)

            mod_val = self.sm(temp) # [B, N, N]
            x = x * mod_val
            x = self.head_list(x)
            return x
            ...
            csi = self.bm_noise_modulator(rate_batch).unsqueeze(1).expand(-1, H * W // (self.num_layers ** 4), -1)
            temp = self.bm_rise_dim(x.detach()) * csi # [B, N, N], N=H x W/(self.num_layers**4)

            mod_val = self.bm(temp) # [B, N, N]
            x = x * mod_val
            ...
            mod_val_sm = self.sm(temp) # [B, N, N]
            x = x * mod_val_sm

            csi = self.bm_noise_modulator(rate_batch).unsqueeze(1).expand(-1, H * W // (self.num_layers ** 4), -1)
            temp = self.bm_rise_dim(x.detach()) * csi # [B, N, N], N=H x W/(self.num_layers**4)

            mod_val_bm = self.bm(temp) # [B, N, N]
            x = x * mod_val_bm
            

            mask = torch.sum(mod_val_bm, dim=1)
            sorted, indices = mask.sort(dim=1, descending=True)
            c_indices = indices[:, :rate]
            add = torch.Tensor(range(0, B * x.size()[2], x.size()[2])).unsqueeze(1).repeat(1, rate)
            c_indices = c_indices + add.int().cuda()
