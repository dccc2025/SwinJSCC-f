...
def get_loader(args, config):
    if args.trainset == 'CIFAR10' and args.training:
        ...
        test_loader = data.DataLoader(dataset=test_dataset, batch_size=1024, shuffle=False)

    elif args.trainset == 'CIFAR10' and not args.training: 
        train_dataset = dataset_(root=config.train_data_dir, train=True, transform=transform_train, download=True)
        def worker_init_fn_seed(worker_id):
            seed = 10
            seed += worker_id
            np.random.seed(seed)
